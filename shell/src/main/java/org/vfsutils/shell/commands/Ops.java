...
public class Ops extends AbstractCommand implements CommandProvider {
	...
	public Ops() {
		super("ops", new CommandInfo("Interact with operations", "list <file> | do <id|name> <file> | usage <id|name> <file>"));
	}
	...
	protected void doop(Arguments args, Engine engine) throws IllegalArgumentException, FileSystemException {
	...
	protected void doop(FileObject file, int index, Arguments args, Engine engine) throws FileSystemException {
	...
	protected void doop(FileObject file, String displayname, Arguments args, Engine engine) throws FileSystemException {
    	...
    	try {
    		...
    		throw new FileSystemException("Error in operation " + displayname, e);
    	}
    }
	
	
	protected void doop(FileOperation op, Arguments args, Engine engine) throws FileSystemException {
	...
	protected Class nameToOperationClass ( Class[] ops, String displayname ) throws IntrospectionException {
		Class opClass = null;
			
		for (int i=0; i<ops.length; i++) {
			Class o = ops[i];
			BeanInfo binfo = java.beans.Introspector.getBeanInfo(o);
			if (binfo.getBeanDescriptor().getDisplayName().equalsIgnoreCase( displayname )) {
				opClass = o;
				break;
			}
		}
    ...
    protected void listops(Arguments args, Engine engine) throws IllegalArgumentException, FileSystemException {
    ...
    protected void listops(FileObject file, Engine engine) throws FileSystemException {
    	
        if (!file.exists()) {
        	engine.println("File does not exist: " + engine.toString(file));
        }
        else {
        	...
        	if (operations != null) {
        		try {
	        		for (int i=0; i < operations.length; i++) {
	        			BeanInfo binfo = java.beans.Introspector.getBeanInfo(operations[i]);
	        			engine.println("[" + (i+1) + "] " + binfo.getBeanDescriptor().getDisplayName());
	        		}
        		}
        		catch (IntrospectionException e) {
        			throw new FileSystemException("Error listing the operations", e);
        		}
        	}
        }
    }
    ...
    protected void usage(Arguments args, Engine engine) throws FileSystemException {
    ...
    protected void usage(FileObject file, String displayname, Engine engine) throws FileSystemException {
    	try {
    		FileOperations fops = file.getFileOperations();
    		Class[] ops = fops.getOperations();
    		...
    		if (opClass == null) {
    			throw new IllegalArgumentException( "Could not find operation " + displayname );
    		}
    		else  {
    		...
    		throw new FileSystemException("Could get usage of operation " + displayname, e);
    	}
    }
    
    protected void usage(FileObject file, int index, Engine engine) throws FileSystemException {
    	...
    	try {
			...
			if (index>0 && index <= ops.length ) {
				...
				throw new IllegalArgumentException("Operation at index " + index + " does not exist");
			}
    	}
    	catch (IntrospectionException e) {
			throw new FileSystemException("Error showing usage of operation at index " + index, e);
		}
    }
	
    protected void usage(Class opClass, Engine engine) throws IntrospectionException {
		...
		for( int i=0; i<props.length; i++ ) {
			...
			if (p.getWriteMethod() != null) {
				engine.println( "  --" + p.getDisplayName() + ": " + p.getShortDescription() );
			}
		}
    }
    
    protected void setOptions(FileOperation op, Arguments args) throws Exception {
		...
		for( int i=0; i<props.length; i++ ) {
			...
			if (p.getWriteMethod() != null) {
				...
				if (args.hasOption(name)) {
					...
					writeMethod.invoke(op, new Object[]{args.getOption(name)});					
				}
			}
		}
    }
