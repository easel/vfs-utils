...
public class Open extends AbstractOpenClose {
	...
	protected void listOpen(Engine engine) throws FileSystemException {
		...
		for (int i=0; i<openFs.size(); i++) {
			...
			engine.println(" "+ (i>8?"":" ") + "[" + (i+1) + "] " + name.getURI());
		}
	}
	
	protected void open(String path, boolean askUsername, boolean askPassword, boolean askDomain, Engine engine) throws FileSystemException {
		...
		//If the new fs is not layered add the current fs in the list to go to when the new fs is closed
