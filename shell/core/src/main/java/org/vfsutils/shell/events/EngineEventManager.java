...
public class EngineEventManager {
	...
	public synchronized void removeEngineEventListener(
	...
	public synchronized void fireCommandStarted(Arguments args) {
		...
		while (iterator.hasNext()) {
			EngineEventListener listener = (EngineEventListener) iterator
					.next();
	...
	public synchronized void fireCommandFailed(Arguments args, Exception e) {
		Iterator iterator = eventListeners.iterator();
		while (iterator.hasNext()) {
			EngineEventListener listener = (EngineEventListener) iterator
					.next();
			listener.commandFailed(args, e);
		}
	}
